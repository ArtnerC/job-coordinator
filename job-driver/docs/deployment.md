...
This guide covers deploying the Job Driver to production environments.
...
The project includes a multi-stage Dockerfile for optimized builds (Go 1.25-alpine):
...
Image size: ~99.4MB (optimized multi-stage build)

### Run Docker Container

**Stdout Mode** (whole file batching):
```bash
docker run -it \
  -v /data/bundles:/data/bundles \
  -v /data/measures:/data/measures \
  -p 8080:8080 \
  job-driver:latest \
...
**Cloud Storage with Gzip** (requires GCP credentials):
```bash
docker run -d \
...
services:
  job-driver:
    ...
    image: job-driver:latest
    container_name: job-driver
    ports:
      - "8080:8080"
    volumes:
      ...
      - ./data/output:/output
    environment:
      - JOB_DRIVER_BASE_PATH=/data/bundles
      - JOB_DRIVER_MEASURES_PATH=/data/measures
      - JOB_DRIVER_DISTRIBUTOR_TYPE=file
      - JOB_DRIVER_DISTRIBUTOR_CONFIG=output_path=/output/work-units.ndjson
      - JOB_DRIVER_AUTO_START=true
...
```bash
gcloud builds submit --tag gcr.io/YOUR_PROJECT_ID/job-driver
```

**Option 2: Local Build + Push**:
```bash
docker build -t gcr.io/YOUR_PROJECT_ID/job-driver .
docker push gcr.io/YOUR_PROJECT_ID/job-driver
```
...
gcloud run deploy job-driver \
  --image gcr.io/YOUR_PROJECT_ID/job-driver \
  --platform managed \
  --region us-central1 \
  ...
  --cpu 2 \
  --timeout 3600 \
  --set-env-vars "JOB_DRIVER_JOB_ID=cloudrun-job" \
  --set-env-vars "JOB_DRIVER_BASE_PATH=/data/bundles" \
  --set-env-vars "JOB_DRIVER_MEASURES_TO_RUN=/measures/measure1.json" \
  --set-env-vars "JOB_DRIVER_DISTRIBUTOR_TYPE=pubsub" \
  --set-env-vars 'JOB_DRIVER_DISTRIBUTOR_CONFIG={"project_id":"YOUR_PROJECT_ID","topic":"work-units"}' \
  --set-env-vars "JOB_DRIVER_AUTO_START=false"
```
...
**Autoscaling**:
```bash
gcloud run services update job-driver \
  --min-instances 0 \
  --max-instances 10 \
  --concurrency 80
```
...
gcloud run deploy job-driver --no-traffic --tag v2

# Gradually shift traffic
gcloud run services update-traffic job-driver --to-revisions v2=50

# Rollback if needed
gcloud run services update-traffic job-driver --to-revisions v1=100
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: coordinator
        image: gcr.io/YOUR_PROJECT_ID/job-driver:latest
        ports:
        - containerPort: 8080
          name: http
        env:
        - name: JOB_DRIVER_JOB_ID
          value: "k8s-job"
        - name: JOB_DRIVER_BASE_PATH
          value: "/data/bundles"
        - name: JOB_DRIVER_MEASURES_TO_RUN
          value: "/measures/measure1.json"
        - name: JOB_DRIVER_DISTRIBUTOR_TYPE
          value: "pubsub"
        - name: JOB_DRIVER_DISTRIBUTOR_CONFIG
          value: '{"project_id":"YOUR_PROJECT_ID","topic":"work-units"}'
        - name: JOB_DRIVER_AUTO_START
          value: "false"
        resources:
          requests:
            memory: "1Gi"
...
```bash
# Check pod status
kubectl get pods -l app=job-driver

# View logs
kubectl logs -l app=job-driver -f

# Check service
kubectl get svc job-driver
```
...
Create `/etc/systemd/system/job-driver.service`:
...
Type=simple
User=coordinator
WorkingDirectory=/opt/job-driver
ExecStart=/opt/job-driver/coordinator \
...
# Install binary
sudo mkdir -p /opt/job-driver
sudo cp coordinator /opt/job-driver/
sudo chown -R coordinator:coordinator /opt/job-driver

# Enable and start service
sudo systemctl daemon-reload
sudo systemctl enable job-driver
sudo systemctl start job-driver

# Check status
sudo systemctl status job-driver
```

## Environment Variables

All CLI flags can be set via environment variables with `JOB_DRIVER_` prefix:

```bash
export JOB_DRIVER_JOB_ID=my-job
export JOB_DRIVER_BASE_PATH=/data/bundles
export JOB_DRIVER_MEASURES_TO_RUN=/measures/measure1.json
